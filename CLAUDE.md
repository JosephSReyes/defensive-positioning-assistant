...
**Do not mention Claude, AI tools, or Anthropic anywhere in the codebase, comments, commit messages, or documentation that gets pushed to GitHub.**
...
- Commit messages must not mention Claude, AI tools, or automation.
...
- Load order in `index.html` must be: storage → utils → game → recommendations → ai → ui → roster-import → play-log-import → memberstack → batter → spray-charts → app.
